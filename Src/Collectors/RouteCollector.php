...
class RouteCollector extends DataCollector implements Renderable
{
     ...
     *
     * @var \Nettixcode\Framework\Http\Request
     */
    protected $request;
    ...
    public function collect()
    {
        ...
        if ($currentRoute) {
            return $this->getRouteInformation($currentRoute);
        } else {
            return [];
        }
    }
    ...
    protected function getRouteInformation($route)
    {
        ...
        if ($this->request->getIlluminateRequest()->hasHeader('X-Livewire')) {
            try {
                $component = $this->request->getIlluminateRequest()->input('components')[0];
