...
abstract class BaseModel extends Model
{
    ...
    {    
        ...
        try {
            ...
            if ($this->operation === 'create') {
                ...
                debug_send('messages',($record ? 'success' : 'error').' '.$this->operation.' to '.$table. ' with data '.json_encode($data));
            ...
            if ($this->operation === 'edit') {
                ...
                debug_send('messages',($result ? 'success' : 'error').' '.$this->operation.' to '.$table. ' with data '.json_encode($data));
            ...
            if ($response) {
                return response()->json(['report' => 'error']);
            } else {
    ...
    protected static function remove($data)
    {
        ...
        try {
            ...
            debug_send('messages',($result ? 'success' : 'error').' remove from '.$table.' with data '.json_encode($data));
            ...
            $connection->rollBack();
            debug_send('exceptions',$e);
            debug_send('messages','error remove from '.$table.' with data '.json_encode($data).' - Exception: '.$e->getMessage());
